...
function getVehiclesCostInCreditsSumTotal(character) {
  ...
  for(i = 0; i < character.vehicles.length; i++){
    if (character.vehicles[i].cost_in_credits !== null ){
     sum = sum + character.vehicles[i].cost_in_credits 
    }
...
function getStarshipPassengerAndCrewSumTotal(character) {
  ...
  for(i = 0; i < character.starships.length; i++){
    ...
    sum = sum + (character.starships[i].crew + character.starships[i].passengers)
    }
  }
  return sum
}
...
function getNthFilm(character, filmNumber) {
  if(filmNumber === 1) {
    ...
    throw `Didn't "They Live" teach you anything?`
  }
}

try {
  console.log(getNthFilm(lukeSkywalker, 4));
}catch (e) {
  console.error(e);
}
